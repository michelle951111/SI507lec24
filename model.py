...
def init_football(csv_file_name=FOOTBALL_FILE_NAME):
    global football_seasons
    with open(csv_file_name) as f:
            reader = csv.reader(f)
            next(reader) # throw away headers
            next(reader) # throw away headers
            global football_seasons
            football_seasons = [] # reset, start clean
            for r in reader:
                        r[3] = int(r[3])
                        r[4] = int(r[4])
                        r[5] = int(r[5])
                        r[6] = float(r[6])
                        football_seasons.append(r)
...
def get_football_seasons(sortby='year', sortorder='desc'):
    ...
    rev = (sortorder == 'desc')
    sorted_list = sorted(football_seasons, key=lambda row: row[sortcol], reverse=rev)
    return sorted_list
